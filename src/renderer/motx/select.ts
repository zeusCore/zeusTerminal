...
export default (motx: MotX) => {
    ...
    motx.subscribe('currentIndex@change', (val) => {
        if (val > -1) {
            ...
            const list = motx.getState(
                leftSide ? 'recommendList' : 'historyList'
            )
    ...
    motx.subscribe('xterm-keydown', ({ code, keyCode }) => {
        console.log('xterm-keydown', code)
